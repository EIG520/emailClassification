input("Make sure to put the email you would like to classify in the email.txt file in the same directory as this file.(note that it can't be too long, a couple thousand characters at most)\nPress enter to continue...\n")
...
with open("email.txt",mode = "rb") as email:
    em = email.read().decode(errors='ignore')

em = tokenizer.texts_to_sequences([em])
em = pad_sequences(em, padding=pad_type, truncating=trunc_type, maxlen=maxlen)

prediction = model.predict(em)[0]
print(prediction)
if(prediction[0] < prediction[1]):
    print("The email is a spam email.")
else:
    print("The email is a ham/not spam email.")